...
- **Optional user message presentation** with bordered and label-only styles, markdown-aware rendering, and safer ANSI/background handling
...
| `enableNativeUserMessageBox` | boolean | `true` | Enable custom user prompt presentation |
| `userMessageStyle` | string | `"bordered"` | `bordered` or `label-only` |
...
When enabled, `bordered` renders user prompts inside a complete labeled outline. The `label-only` style keeps Pi's native prompt card, paints it with the same neutral-charcoal background as the custom live input, and places a compact `user` label inside it without an outline. The bordered renderer preserves markdown content more safely and normalizes ANSI/background handling to avoid odd nested background artifacts.
